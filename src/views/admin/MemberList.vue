    ...
    <div>
      <b-table striped hover :fields="fields" :items="items" @row-clicked="boardDetail">
        ...
        <template #cell(name)="data">
          {{data.item.name}}
          <router-link :to="{name: 'Member', params:{userid: data.item.userid}}"></router-link>
        </template>
      </b-table>
...
export default {
  ...
  methods:{
    searchList(){
        this.$axios.post('member/searchList'
      ...
      boardDetail(item){
        this.$router.push({name: 'Member',params:{userid:item.userid,gubun:'ADMIN'}});
      }
    }
  }
</script>